    ...
    internal class Commands
    {
        ...
        public void __rkqopzla947()
        {
            ...
            if (_document != null)
            {
                ...
                options.Filter = "RPSOL files (*.rpsol)|*.rpsol";
                ...
                if (result.Status == PromptStatus.OK)
                {
                    ...
                    RPConfig config = ConfigController.LoadConfig<RPConfig>(lsFile, lsPath);
                    if (config != null)
                    {
                        _document.Editor.WriteMessage(string.Format("Changed current project to: {0}\n", config?.LastRoute));
                        RPApp.RDPHelper.CreateConfigRDP(lsPath, config?.LastRoute);
                    }
                }
            }
        }
